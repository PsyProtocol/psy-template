 ...
 * 
 * Verifies the 5 core adversarial scenarios for the Conditional Dual-Ledger
 * Delegation Model with explicit Cutoff & Finality constraints.
 * 
 * Invariants:
...
test('Delegation 5-Case Adversarial Verification Suite', async (t) => {
  ...
  await t.test('Case 6: Real .psy Compiled Contract Artifacts & Dual-Ledger Delegation ABI Verification', async () => {
    ...
    // 1. Verify Dual-Ledger Storage Layout in compiled contract (14 storage fields, state_map for nullifiers and spend records)
    const stateNames = tokenAbi.contract.state.map(s => s.name);
    assert.equal(tokenAbi.contract.state.length, 14, 'Contract state must have exactly 14 fields');
    assert.ok(stateNames.includes('delegations'), 'Storage must include delegations');
    assert.ok(stateNames.includes('state_map'), 'Storage must include state_map (NS 1 Nullifiers & NS 2 Spender Ledger)');
    assert.ok(!stateNames.includes('delegation_records'), 'Single-slot delegation_records array must be removed in favor of state_map NS 2');
    ...
    // 3. Verify Two-Phase Revocation inputs match cutoff specification
