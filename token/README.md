...
The existing `.psy` `private_claim` verifies a note-inclusion fingerprint against the session proof tree root and binds its checkpoint user tree root before crediting a balance and recording a nullifier. The chain's built-in token artifact already contains both private methods; the protocol and wallet support private transfers. **No node change is required to invent or enable private transfers.** The template-specific gap is compilation and user-deployment compatibility: installed `dargo` produces a method-definition array, while the node-matched user CLI expects a layout-aware compilation artifact. Wrapping the current array and ABI gets past parsing but fails local circuit generation with `Invalid target index`. The installed 0.1.1 and 0.1.2 v3 compilers reject the legacy proof-root intrinsic with `Unknown function`. The immediate work is to use a matching compiler/deployment toolchain for the complete `.psy` source or port it to a compatible v3 compiler, then verify a real two-user private claim. Adding only a deposit to v3 would debit users without a secure claim path. Existing deployments need a separate migration plan.
