...
process.env.PSY_USER_CLI = cli;
const temporary = mkdtempSync(join(tmpdir(), 'psy-token-v3-deploy-'));
...
try {
  ...
  run(process.execPath, [join(project, 'scripts/build_private_v3.mjs')], 'PSY-20 private v3 compilation');
  const resultFile = join(temporary, 'deploy.json');
  run(cli, ['deploy-contract', '--contract-path', join(project, 'target/private-v3/compilation_artifact.json'),
    '--is-deploy', '--result-file', resultFile], 'PSY-20 private v3 deployment');
  ...
  const record = { network: result.network, tx_hash: result.tx_hash, status: result.status,
    contract_id: result.contract_id ?? null, source: 'src/main.private.psy.rs' };
  ...
  writeFileSync(join(project, '.psy-deploy-private-v3.json'), JSON.stringify(record, null, 2) + '\n');
  console.log(`PSY-20 private v3 deployment ${record.status}: ${record.tx_hash}`);
  if (record.contract_id === null) {
