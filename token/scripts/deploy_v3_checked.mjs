...
try {
  ...
  run(process.execPath, [join(project, 'scripts/build_v3.mjs')], 'PSY-20 v3 compilation');
  ...
  run(cli, ['deploy-contract', '--contract-path', join(project, 'target/v3/compilation_artifact.json'),
    '--is-deploy', '--result-file', resultFile], 'PSY-20 v3 deployment');
  ...
  console.log(`PSY-20 v3 deployment ${record.status}: ${record.tx_hash}`);
  if (record.contract_id === null) {
